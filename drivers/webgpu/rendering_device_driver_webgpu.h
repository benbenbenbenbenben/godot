...
class RenderingDeviceDriverWebGPU : public RenderingDeviceDriver {
	RenderingContextDriverWebGPU *context_driver;
	RenderingDeviceDriver::Capabilities capabilities;
    ...
    struct WebGPUSwapChain {
        ...
        // The current texture acquired from the swapchain for the current frame
        WebGPUTexture *current_texture = nullptr; 
        // We might need a persistent Framebuffer ID that wraps the changing swapchain texture?
        // Or we return a new FramebufferID every frame?
        // RDD expects swap_chain_acquire_framebuffer to return a FramebufferID.
        // We can create a ephemeral framebuffer or one associated with the swapchain.
        FramebufferID current_framebuffer_id;
        WebGPUFramebuffer *current_framebuffer_ptr = nullptr;
        RenderPassID default_render_pass;
    };

    struct WebGPURenderPass {
        // We store attachment formats and load/store ops here
        Vector<Attachment> attachments;
        Vector<Subpass> subpasses;
        // Helper to store WGPURenderPassColorAttachment descriptors if possible/needed pre-calculated
    };

    struct WebGPUCommandPool {
        CommandQueueFamilyID queue_family;
        CommandBufferType type;
    };

    struct WebGPUCommandBuffer {
        WGPUCommandEncoder encoder = nullptr; // Valid during recording
        WGPUCommandBuffer buffer = nullptr;   // Valid after recording (end)
        WebGPUCommandPool *pool = nullptr;
    ...
    struct WebGPUShader {
        // Map stage to module
        HashMap<RenderingDeviceDriver::ShaderStage, WGPUShaderModule> modules;
        
        Vector<WGPUBindGroupLayout> bind_group_layouts;
        
        struct BindingInfo {
            uint32_t wgpu_binding = 0;
            uint32_t mirror_binding = 0xFFFFFFFF;
        };
        struct SetInfo {
             HashMap<uint32_t, BindingInfo> bindings;
        };
        Vector<SetInfo> reflection_info;
    };

    struct WebGPUVertexFormat {
        Vector<WGPUVertexBufferLayout> buffers;
        Vector<WGPUVertexAttribute> attributes;
        // We need to keep the attribute arrays alive if they are referenced by pointer in buffers?
        // WGPUVertexBufferLayout has 'attributes' pointer.
        // So we strictly need to store valid Vectors of attributes for each buffer layout.
        // Let's store:
        struct BufferLayout {
    ...
    PagedAllocator<WebGPUSwapChain> swap_chain_allocator;
    PagedAllocator<WebGPUFramebuffer> framebuffer_allocator;
    PagedAllocator<WebGPURenderPass> render_pass_allocator;
    PagedAllocator<WebGPUCommandPool> command_pool_allocator;
    PagedAllocator<WebGPUCommandBuffer> command_buffer_allocator;
    PagedAllocator<WebGPUShader> shader_allocator;
    PagedAllocator<WebGPUVertexFormat> vertex_format_allocator;
    PagedAllocator<WebGPUPipeline> pipeline_allocator;
    PagedAllocator<WebGPUUniformSet> uniform_set_allocator;
...
public:
	...
	virtual void command_pipeline_barrier(CommandBufferID p_cmd_buffer, BitField<PipelineStageBits> p_src_stages, BitField<PipelineStageBits> p_dst_stages, VectorView<MemoryAccessBarrier> p_memory_barriers, VectorView<BufferBarrier> p_buffer_barriers, VectorView<TextureBarrier> p_texture_barriers) override;
